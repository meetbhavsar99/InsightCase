...
export class UnauthorizedExceptionFilter implements ExceptionFilter {
    ...
    if (!this.validateRedirectHostname(afterLoginRedirect, request)) {
      console.error('400 Bad Request: Invalid request hostname: ' + request.hostname);
      throw new BadRequestException('Invalid request hostname: ' + request.hostname);
    }
  ...
  private validateRedirectHostname(afterLoginRedirectUrl: string, req: Request): boolean {
